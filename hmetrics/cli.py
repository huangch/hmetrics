...
import argparse
import pandas as pd
from hmetrics.runners import run_hmetrics_batch
@click.group(context_settings=dict(help_option_names=["-h", "--help"]))
@click.version_option(package_name="hmetrics")
def main():
...
@click.option("--out", "-o", type=click.Path(dir_okay=False, path_type=str),
              default="hmetrics.png", show_default=True, help="Output plot file.")
...
#     plt.tight_layout()
#     plt.savefig(out)
#     click.echo(f"[hmetrics] saved → {out}")
...
def main():
    ...
    parser.add_argument("--output_dir", default="hmetricss", help="Directory to save plots")
    parser.add_argument("--file_prefix", default="hmetrics", help="Prefix for output file names")
